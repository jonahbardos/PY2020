...
private:
	...
	/**
	 * Updates the angle of the lidar based on the time that has passed since last it generated
	 * a point and the average speed.
	 */
	bool updateAngle();
	// Returns the angle which should be used in a cosine equation to convert to square
	// coordinates.
	double actingAngle();
	// Copies the currentFrame into lastFrame and then clears the currentFrame so it can be
	// built again.
	void createFrame();
	std::chrono::time_point<std::chrono::steady_clock>
		last;		  // The current system time in milliseconds
	...
	std::vector<std::shared_ptr<Polar2D>>
		currentFrame; // The frame that is currently being built.
	std::chrono::time_point<std::chrono::steady_clock>
		lastFrameTime; // The system time at which the last frame was completed.

public:
	SyntheticLidar();
	/**
	 * Steps the Lidar for the time having passed since it was last called (Given the average
	 * time for a complete rotation of the lidar) and returns the next polar coordinate at the
	 * given radius for a circle.
	 *
	 ...
	 * Steps the Lidar for the time having passed since it was last called (Given the average
	 * time for a complete rotation of the lidar) and returns the next polar coordinate at the
	 * given dimension for a axis-aligned square.
	 *
