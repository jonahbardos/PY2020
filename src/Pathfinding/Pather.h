...
public:
	...
	Pather::Pather(std::vector<std::vector<bool>> map)
		...
		private : std::vector<std::vector<bool>> map;
	struct point
	{
		int x;
		int y;
	} std::vector<point> visited struct queueElement
	{
		point previousNode; // how to use point struct in here?
		point activeNode;
		int distFromSrc;
		bool operator<(const queueElement &locate1) const
		{
			return !(distFromSrc < locate1.disFromSrc);
		}
	}
};
