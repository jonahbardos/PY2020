...
private:
	// the dummy chunk is a default reference to return if we can't give a valid chunk. (check
	// in_bounds!!!!)
...
public:
	...
	findObjectsWithinRadius(float radius, float x, float y) const;
	std::vector<std::shared_ptr<const MapObstacle>>
	findObjectsWithinSquare(float half_width, float x, float y) const;
	std::vector<std::shared_ptr<const MapObstacle>>
	findObjectsWithinRect(float x_lower_left, float y_lower_left, float x_upper_right,
						  float y_upper_right) const;
...
private:
	...
	void iterateChunkRect(XYQ low, XYQ high,
