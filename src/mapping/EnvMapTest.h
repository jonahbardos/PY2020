...
#define EMT_TESTSET_END                                                                       \
	...
	if (__fail_count != 0)                                                                    \
	{                                                                                         \
		std::cout << "In " << __test_set_name << ", " << __passed_count << " passed, "        \
				  << __fail_count << " failed out of " << __test_count << " total."           \
				  << std::endl;                                                               \
	}                                                                                         \
	else                                                                                      \
	{                                                                                         \
		std::cout << "In " << __test_set_name << ", all tests (" << __test_count              \
				  << ") passed." << std::endl;                                                \
	}                                                                                         \
	std::cout << std::endl;                                                                   \
	}
...
#define EMT_ASSERT_NOTHROW_BEGIN                                                              \
	{                                                                                         \
		const std::string __nothrow_test_name =                                               \
...
#define EMT_ASSERT_NOTHROW_END                                                                \
	...
	{                                                                                         \
		__test_failed = true;                                                                 \
		__test_messages.push_back(std::string(__FILE__) + std::string(", ") +                 \
								  std::to_string(__LINE__) +                                  \
								  ": ASSERT_NOTHROW failed: test threw an exception.");       \
	}                                                                                         \
	}
...
#define EMT_ASSERT_EQ(tested, expected)                                                       \
	{                                                                                         \
		...
		{                                                                                     \
			...
			__test_failed = true;                                                             \
			__test_messages.push_back(std::string(__FILE__) + std::string(", ") +             \
									  std::to_string(__LINE__) + ": ASSERT_EQ failed: \"" +   \
									  _EMT_STRINGIFY(tested) + "\" was not equal to \"" +     \
									  _EMT_STRINGIFY(expected) + "\"");                       \
		}                                                                                     \
	}
